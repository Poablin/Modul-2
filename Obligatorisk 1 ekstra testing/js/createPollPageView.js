function updateViewCreateVotePage() {
    ...
    const buttonText = model.app.currentPoll === model.inputs.createPoll.pollId ? 'Oppdater' : 'Lag';
    const buttonText2 = model.app.currentPoll === model.inputs.createPoll.pollId ? '<button onclick="startNewPoll()"> Ny? </button>' : '';
    ...
    document.getElementById('app').innerHTML = `        
        ...
        <p>&nbsp;</p>
        <button onclick="createOrUpdatePoll()" style="font-size: 150%">${buttonText} spørreundersøkelse</button> ${buttonText2} 
